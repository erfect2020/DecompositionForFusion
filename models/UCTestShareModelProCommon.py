...
class UCTestSharedNetPro(nn.Module):
    def __init__(self):
        ...
        self.decoder_common_layer1 = ResNestLayer(Bottleneck, 64, 2, stem_width=768, avg_down=False, avd=False, stride=1, norm_layer=None)
        self.decoder_common_up1 = nn.Upsample(scale_factor=2, mode='bilinear') # nn.PixelShuffle(2)
        self.decoder_common_layer2 = ResNestLayer(Bottleneck, 16, 2, stem_width=256, avg_down=False, avd=False, stride=1, norm_layer=None)
        self.decoder_common_up2 = nn.Upsample(scale_factor=2, mode='bilinear') # nn.PixelShuffle(2)
        self.decoder_common_layer3 = ResNestLayer(Bottleneck, 4, 2, stem_width=96, avg_down=False, avd=False, stride=1, norm_layer=None)
    ...
    def forward(self, img1, img2):
        ...
        common_part = self.decoder_common_layer1(common_part)
        ...
        # for i in range(16):
        #     common_part[:, i] = common_part[:, i] / common_part[:, i].max()
        #     upper_part[:, i] = upper_part[:, i] / upper_part[:, i].max()
        #     lower_part[:, i] = lower_part[:, i] / lower_part[:, i].max()
        common_part = self.decoder_upper_projection_layer(common_part_embedding)
        upper_part = self.decoder_upper_projection_layer(upper_part_embeding)
        lower_part = self.decoder_lower_projection_layer(lower_part_embeddding)
        ...
        return common_part, upper_part, lower_part, fusion_part
        # return common_part_embedding, common_part, upper_part_embeding, upper_part, lower_part_embeddding, lower_part, fusion_part


class UCTestSharedDeepNetPro(nn.Module):
    def __init__(self):
        super(UCTestSharedDeepNetPro, self).__init__()
        encoder_upper = [nn.Conv2d(3, 16, 3, 1, 1, bias=True),
                         nn.ReLU(inplace=True),
                         ResNestLayer(Bottleneck, 8, 6, stem_width=8, norm_layer=None),
                         ]
...
class UCTestSharedAblaNetPro(nn.Module):
    def __init__(self):
        super(UCTestSharedAblaNetPro, self).__init__()
        encoder_upper = [nn.Conv2d(3, 16, 3, 1, 1, bias=True),
                         nn.ReLU(inplace=True),
                         ResNestLayer(Bottleneck, 8, 6, stem_width=8, norm_layer=None),
                         ]
